 ...
 *
 * Created by Luca Stornaiuolo on 10/01/2019.
 * Copyright © 2019 Politecnico di Milano. All rights reserved.
 */
...
#pragma HLS UNROLL
        ...
        for (int tp = 0; tp < p; tp++)
            for (int tq = 0; tq < q; tq++)
#pragma HLS UNROLL
                ;//linearRegisters[tp * q + tq][A_custom(i, j, tp * q + tq, POLYMEM_ACCESS_TYPE)] = tmp[tp][tq];
...
#pragma HLS UNROLL
        ...
        for (int t = 0; t < N_LINEAR_REGISTERS; t++)
#pragma HLS UNROLL
            ;//linearRegisters[t][A_custom(i, j, t, POLYMEM_ACCESS_TYPE)] = mask_tmp[t] ? tmp[t] : linearRegisters[t][A_custom(i, j, t, POLYMEM_ACCESS_TYPE)];
    }
...
#pragma HLS UNROLL
                switch (POLYMEM_ACCESS_TYPE) {
					case POLYMEM_ACCESS_TYPE_Ro:
						out[k * q + l] = tmp[m_v(i, j + k * q + l)][m_h(i, j + l)];
						break;
					case POLYMEM_ACCESS_TYPE_Co:
						out[l * p + k] = tmp[m_v(i + k, j)][m_h(i + l * p + k, j)];
						break;
					case POLYMEM_ACCESS_TYPE_Re:
						out[k * q + l] = tmp[m_v(i + k, j + l)][m_h(i + k, j + l)];
						break;
					case POLYMEM_ACCESS_TYPE_Tr:
						out[k * q + l] = tmp[m_v(i + k, j + l)][m_h(i + k, j + l)];
						break;
					case POLYMEM_ACCESS_TYPE_MD:
						out[k * q + l] = tmp[m_v(i + l, j + k * q + l)][m_h(i + l + k * q, j + l)];
						break;
					case POLYMEM_ACCESS_TYPE_SD:
						out[k * q + l] = tmp[m_v(i + l, j - k * q - l)][m_h(i + l + k * q, j - l)];
						break;
...
#pragma HLS UNROLL
                switch (POLYMEM_ACCESS_TYPE) {
					case POLYMEM_ACCESS_TYPE_Ro:
						tmp2[k][l] = tmp[k][m_h(i, j + l)];
						break;
					case POLYMEM_ACCESS_TYPE_Co:
						tmp2[k][l] = tmp[k][m_h(i + l * p + k, j)];
						break;
					case POLYMEM_ACCESS_TYPE_Re:
						tmp2[k][l] = tmp[k][m_h(i + k, j + l)];
						break;
					case POLYMEM_ACCESS_TYPE_Tr:
						tmp2[k][l] = tmp[k][m_h(i + k, j + l)];
						break;
					case POLYMEM_ACCESS_TYPE_MD:
						tmp2[k][l] = tmp[k][m_h(i + l + k * q, j + l)];
						break;
					case POLYMEM_ACCESS_TYPE_SD:
						tmp2[k][l] = tmp[k][m_h(i + l + k * q, j - l)];
						break;
...
#pragma HLS UNROLL
                switch (POLYMEM_ACCESS_TYPE) {
					case POLYMEM_ACCESS_TYPE_Ro:
						out[k * q + l] = tmp2[m_v(i, j + k * q + l)][l];
						break;
					case POLYMEM_ACCESS_TYPE_Co:
						out[l * p + k] = tmp2[m_v(i + k, j)][l];
						break;
					case POLYMEM_ACCESS_TYPE_Re:
						out[k * q + l] = tmp2[m_v(i + k, j + l)][l];
						break;
					case POLYMEM_ACCESS_TYPE_Tr:
						out[k * q + l] = tmp2[m_v(i + k, j + l)][l];
						break;
					case POLYMEM_ACCESS_TYPE_MD:
						out[k * q + l] = tmp2[m_v(i + l, j + k * q + l)][l];
						break;
					case POLYMEM_ACCESS_TYPE_SD:
						out[k * q + l] = tmp2[m_v(i + l, j - k * q - l)][l];
						break;
