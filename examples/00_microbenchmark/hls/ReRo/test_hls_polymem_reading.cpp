...
int main() {
    ...
    for(i = 0; i < LEN_READINGS * 3 / 4; i++){
        min = 0;
        max = DIM - N_LINEAR_REGISTERS;
    	readings[i] = (DATA_T) (min + (int) ((DATA_T) rand() / ((DATA_T) RAND_MAX / (max - min))));
    }
    for(i = LEN_READINGS * 3 / 4; i < LEN_READINGS; i+=2){
        min = 0;
        max = DIM - N_LINEAR_REGISTERS;
    	readings[i] = (DATA_T) (min + (int) ((DATA_T) rand() / ((DATA_T) RAND_MAX / (max - min))));

        min = N_LINEAR_REGISTERS;
        max = DIM;
    	readings[i+1] = (DATA_T) (min + (int) ((DATA_T) rand() / ((DATA_T) RAND_MAX / (max - min))));
    }
